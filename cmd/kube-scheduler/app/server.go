...
func Run(ctx context.Context, cc *schedulerserverconfig.CompletedConfig, sched *scheduler.Scheduler) error {
	...
	if cc.SecureServing != nil {
		handler := buildHandlerChain(newHealthzAndMetricsHandler(&cc.ComponentConfig, cc.InformerFactory, isLeader, sched, ctx, checks...), cc.Authentication.Authenticator, cc.Authorization.Authorizer)
...
func installSchedulerHandler(
	pathRecorderMux *mux.PathRecorderMux,
	sched *scheduler.Scheduler,
	isLeader func() bool,
	ctx context.Context) {
	configz.InstallHandler(pathRecorderMux)
	pathRecorderMux.Handle("/scheduler", legacyregistry.HandlerWithReset())
	pathRecorderMux.HandleFunc("/scheduler/vkfilterpod", func(w http.ResponseWriter, req *http.Request) {
		if !isLeader() {
			return
		}
		pod := &v1.Pod{}
		...
		//vkFilterPod := sched.SchedulePod
		nodeNames, err := sched.VkFilterPod(ctx, pod)
		nodeName, err := json.Marshal(nodeNames)
		if err != nil {
		...
		if nodeNames.SuggestedHost == "" {
			w.WriteHeader(http.StatusOK)
			w.Write([]byte("no node to sche"))
			return
		}
...
func newSchedulerHandler(recorderMux *mux.PathRecorderMux, factory informers.SharedInformerFactory, isLeader func() bool) {

}

// newHealthzAndMetricsHandler creates a healthz server from the config, and will also
// embed the metrics handler.
func newHealthzAndMetricsHandler(
	...
	installSchedulerHandler(pathRecorderMux, sched, isLeader, ctx)
...
func Setup(ctx context.Context, opts *options.Options, outOfTreeRegistryOptions ...Option) (*schedulerserverconfig.CompletedConfig, *scheduler.Scheduler, error) {
	...
	// Create the scheduler.调用pkg的scheduler的new返回了一个调度器实例
