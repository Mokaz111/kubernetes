...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='4f874a224a31a5dc5745a0c140d3769bb178f811'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'HEAD -> master, refs/__gh__/pull/119973/rebase' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'HEAD -> master, refs/__gh__/pull/119973/rebase' =~ tag:\ (v[^ ,]+) ]]; then
